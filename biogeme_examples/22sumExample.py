########################################
#
# @file 22sumExample.py
# @author: Michel Bierlaire, EPFL
# @date: Sun May 20 15:00:40 2012
#
# This example is the same as 01logit.py, but using the syntax of a
# sum for the denominator of logit. This is less efficient, and should
# not be used in this context. However, it illustrate the use of
# operator bioMultSum
#######################################
...
ASC_TRAIN = Beta('ASC_TRAIN',-1,-10,10,0,'Train cte.')
ASC_SM = Beta('ASC_SM',0,-10,10,1,'Swissmetro cte.')
B_TIME = Beta('B_TIME',-0.01,-10,10,0,'Travel time')
B_COST = Beta('B_COST',-0.01,-10,10,0,'Travel cost')
...
# The choice model is a logit, with availability conditions Note that
# the operator bioLogit should be used, but we illustrate here the use
# of the operator bioMultSum
Vchosen = Elem(V,CHOICE)
d = {}
for i in range(1,4):
    d[i] = av[i]*exp(V[i]-Vchosen)

logprob = Vchosen - log(bioMultSum(d))
